...
// Hardcoded palette so the hero gradient is immune to dynamic color (Android 12+)
private val UserHdrTop    = Color(0xFF071226)
private val UserHdrMid    = Color(0xFF0B1B3A)
private val UserHdrBottom = Color(0xFF1565C0)
private val UserOnHdr     = Color(0xFFEAF2FF)
private val UserOnHdrMuted = Color(0xFF8BAFD4)
...
fun UserProfileScreen(
    ...
    Box(
        ...
        Column(
            ...
            // ── Hero header ───────────────────────────────────────────────────
            // Edge-to-edge: gradient extends behind the status bar, icon/text
            // are pushed down by outerPadding.calculateTopPadding().
            Box(
                modifier = Modifier
                    .fillMaxWidth()
                    .background(
                        Brush.verticalGradient(listOf(UserHdrTop, UserHdrMid, UserHdrBottom))
                    )
                    .padding(
            ...
            Column(
                ...
                UserProfileSection(title = "Account") {
                    ...
                    UserProfileMenuItem(
                        ...
                        onClick = { Toast.makeText(context, "Loyalty Points", Toast.LENGTH_SHORT).show() }
                    )
                    Divider(color = MaterialTheme.colorScheme.outlineVariant)
                    UserProfileMenuItem(
...
@Composable
private fun UserProfileMenuItem(
    icon:    ImageVector,
    label:   String,
    onClick: () -> Unit
) {
